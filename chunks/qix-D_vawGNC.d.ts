import { d as HostConfig } from "./auth-types-yKuw6LLB.js";
import "../auth.js";
//#region src/qix/types/engine-schema.d.ts
declare const QIX_SCHEMA_VERSION = "12.2926.0";
type AlfaNumString = {
  /**
   * Calculated value.
   */
...
type BookmarkFieldVerifyResultState = "NOT_VERIFIED" | "FIELD_VALUE_MATCH_ALL" | "FIELD_MISSING" | "FIELD_VALUE_MISSING" | "STATE_MISSING" | "SEARCH_NO_MATCH";
type BookmarkFieldVerifyWarning = {
  ...
  /**
   * Field/values verfication result *
   * Result of ApplyAndVerify for one field, used only in BookmarkFieldVerifyWarning.
   * One of:
   * * NOT_VERIFIED
  ...
  /**
   * Strings representing missing field values. Will be replaced by MissingDuals. *
   *
   * @deprecated: This will be removed in a future version
   */
  qMissingValues?: string[];
  /**
   * Missing field values as duals, carrying both text and numeric parts. Number of values included is capped. Default cap is 20. *
   *
   * Stability: *experimental*
   */
  qMissingDuals?: FieldValue[];
  /**
   * Total count of missing field values *
   *
   * Stability: *experimental*
   */
  qTotalMissing?: number;
};
/**
 ...
 * * _GOOGLE_SHEET_ for Google Sheet file
 */
type FileDataFormat = {
  /**
   * Type of the file.
   ...
   * * GOOGLE_SHEET or FILE_TYPE_GOOGLE_SHEET
   */
  qType?: FileType;
  /**
   * One of:
...
type GenericVariableConstraints = {
  qType?: GenericVariableType;
  qValuesText?: string[];
  qValuesNum?: number[];
};
/**
...
type GenericVariableType = "any" | "VARIABLE_TYPE_ANY" | "number" | "VARIABLE_TYPE_NUMBER" | "text" | "VARIABLE_TYPE_TEXT";
...
type NxLibraryDimension = {
  ...
  /**
   * ## NxOrigin
   * With the introduction of Data Products, there will be a need to describe the origin of objects other than just being
   * generated by a loading script or not.
   *
   * This enum contains:
   * * PROGRAM:      Object is created via ui or api call
   * * SCRIPT:       Object is created from load script
   * * DATA_PRODUCT: Object is created from a data product
   * One of:
   * * PROGRAM
   * * SCRIPT
   * * DATA_PRODUCT
...
type NxOrigin = "PROGRAM" | "SCRIPT" | "DATA_PRODUCT";
type NxPage = {
  /**
   * Position from the left.
...
type ReloadError = {
  ...
  qLine?: string;
  /**
   * Script line number where error occurred (zero-based).
   */
  qLineNumber?: number;
  /**
   * Error code.
...
type NativeDocFunctions<FieldRpcObject, GenericBookmarkRpcObject, GenericDimensionRpcObject, GenericMeasureRpcObject, GenericObjectRpcObject, GenericVariableRpcObject, DefaultGenericObjectProperties> = {
   ...
   * - `strictFieldValidation`   -   If true, enforces strict validation of field references.
   *
   * Stability: *locked*
   */
  checkExpression: (expr: string, labels?: string[], strictFieldValidation?: boolean) => Promise<unknown>;
  /**
   * Checks if:
...
//#endregion
//#region src/qix/types/list-types.d.ts
...
type VariableListItem = NxVariableListItem<VariableListData>;
...
//#endregion
//#region src/qix/types/rpc-object.d.ts
type RpcObject = {
  id: string;
  type: string;
  genericType?: string;
...
type LayoutSubscriptionCallback<L extends GenericObjectLayout = AnyGenericObjectLayout> = (layout: L) => void;
type LayoutSubscription<L extends GenericObjectLayout = AnyGenericObjectLayout> = {
  subscribe(callback: LayoutSubscriptionCallback<L>): void;
  dispose(): void;
  cancel(): void;
  retry(): void;
};
...
//#endregion
//#region src/qix/types/generic-object.d.ts
...
//#endregion
//#region src/qix/types/global.d.ts
type Global = RpcObject & NativeGlobalFunctions<Doc>;
//#endregion
//#region src/qix/types/doc.d.ts
...
//#endregion
//#region src/qix/types/websocket-events.d.ts
 ...
 */
type CombinedWebSocketStateEvent = {
  state: CombinedWebSocketStateState;
};
//#endregion
//#region src/qix/types/progress.d.ts
type ProgressType = "open" | "reload" | "save";
type QixProgressErrorSeverity = "error" | "warning" | "circular";
...
/**
 * A single progress message from the engine with a message code and parameters.
 * The message code stands for what type of message it is and the parameters carry any
 * textual information. Usually, only the first parameter carries the message text.
 */
type QixProgressMessage = {
  ...
  code: number;
  /**
   * Values to substitute into the localized message. The first parameter holds the message's dynamic
   * text (a table name, row count, connection, error string, etc.); any further parameters are
   * additional substitutions. Always present; defaults to `[""]`.
   */
  parameters: string[];
};
type QixProgressEventBase = {
  /** Which long-running operation this event describes. */
  type: ProgressType;
};
/**
 ...
 *
 * `persistent` is the accumulated log of completed steps (oldest first) and `transient` is the
 * message that is currently changing. A typical display shows every `persistent` message followed
 * by the `transient` one when it differs from the last persistent message.
 */
type QixProgressEvent = (QixProgressEventBase & {
  status: "started";
...
type ProgressListener = (event: QixProgressEvent) => void;
//#endregion
...
type AppSession = {
   ...
   * Note: does not work with registerExternalAppSession.
   */
  onOpenProgress: (listener: (event: QixProgressEvent) => void) => () => void;
  /**
   * @experimental
   ...
   * Note: does not work with registerExternalAppSession.
   */
  onReloadProgress: (listener: (event: QixProgressEvent) => void) => () => void;
  /**
   * @experimental
   * Add a progress-listener for an app save process (triggered by `DoSave` on a `Doc`).
   * Note: does not work with registerExternalAppSession.
   */
  onSaveProgress: (listener: (event: QixProgressEvent) => void) => () => void;
  /**
   * @experimental
 ...
 * @param appId The app id of the app (referred to as resourceId in the items api) or full
 * OpenAppSessionProps configuration
 * @returns The Doc object if loaded, or undefined while loading/on error
 */
...
//#endregion
export { BookmarkListItem as $, NxTreeRangeSelectInfo as $a, NxInlineDimensionDef as $i, FunctionGroup as $n, SearchSuggestionResult as $o, NxAttrDimDef as $r, CodePage as $t, Field as A, NxPatches as Aa, NxDownloadInfo as Ai, ExtendedPivotStateData as An, SearchAttribute as Ao, LayoutExclude as Ar, TotalMode as As, ApplyGroupStatesResult as At, GenericObjectLayoutNodeLevel4 as B, NxSimpleValue as Ba, NxFieldSelectionMode as Bi, FieldList as Bn, SearchGroup as Bo, MediaListItem as Br, VariableList as Bs, BookmarkFieldPage as Bt, ObjectHelperFunctions as C, NxMultiRangeSelectInfo as Ca, NxDerivedFieldDescriptionList as Ci, EditorBreakpoint as Cn, RangeSelectInfo as Co, HyperCubeDef as Cr, TableViewBroomPointSaveInfo as Cs, AppEntry as Ct, AnyGenericObjectLayout as D, NxPageTreeNode as Da, NxDimension as Di, ErrorDataCode as Dn, ScriptSyntaxError as Do, InteractType as Dr, TableViewSaveInfo as Ds, AppScriptMeta as Dt, Global as E, NxPageTreeLevel as Ea, NxDimCellType as Ei, ErrorData as En, SampleResult as Eo, InteractDef as Er, TableViewDlgSaveInfo as Es, AppScript as Et, GenericMeasure as F, NxRangeSelectInfo as Fa, NxFeature as Fi, FieldAttributes as Fn, SearchFieldMatch as Fo, LocaleInfo as Fr, UndoInfo as Fs, BNFType as Ft, GenericObjectPropertiesNodeLevel3 as G, NxStreamListEntry as Ga, NxGroupDef as Gi, FieldValue as Gn, SearchGroupOptions as Go, NativeGenericDimensionFunctions as Gr, BookmarkListDef as Gt, GenericObjectLayoutNodeRec as H, NxStackPage as Ha, NxFrequencyMode as Hi, FieldOrColumn as Hn, SearchGroupItemMatch as Ho, NativeDocFunctions as Hr, BookmarkFieldVerifyResultState as Ht, GenericObject as I, NxSelectionCell as Ia, NxFieldDescription as Ii, FieldDefEx as In, SearchFieldMatchType as Io, LogOnType as Ir, UndoInfoDef as Is, Blob as It, GenericObjectPropertiesNodeRec as J, NxTreeDataOption as Ja, NxGroupTail as Ji, FilterInfo as Jn, SearchMatchCombinations as Jo, NativeGenericVariableFunctions as Jr, CalendarStrings as Jt, GenericObjectPropertiesNodeLevel4 as K, NxTempBookmarkOptions as Ka, NxGroupMemberClass as Ki, FileDataFormat as Kn, SearchGroupType as Ko, NativeGenericMeasureFunctions as Kr, BookmarkStateFieldPages as Kt, GenericObjectLayoutNode as L, NxSelectionCellType as La, NxFieldProperties as Li, FieldDescription as Ln, SearchFieldMatchesItem as Lo, MeasureList as Lr, UsageEnum as Ls, Bookmark as Lt, GenericBookmark as M, NxPivotPage as Ma, NxEngineVersion as Mi, ExtensionListDef as Mn, SearchCombinationOptions as Mo, LineageInfo as Mr, TransformAppResult as Ms, AssociationScore as Mt, GenericDimension as N, NxPivotValuePoint as Na, NxExportFileType as Ni, FailureData as Nn, SearchContextType as No, ListObject as Nr, TreeData as Ns, BNFDef as Nt, AnyGenericObjectProperties as O, NxPatch as Oa, NxDimensionInfo as Oi, ExpansionData as On, ScrollPosition as Oo, KeyType as Or, TableViewTableWinSaveInfo as Os, ApplyGroupStateWarning as Ot, GenericListObject as P, NxRange as Pa, NxExportState as Pi, FieldAttrType as Pn, SearchFieldDictionary as Po, ListObjectDef as Pr, TreeDataDef as Ps, BNFDefMetaType as Pt, BookmarkListData as Q, NxTreeNode as Qa, NxInfo as Qi, FrequencyDistributionData as Qn, SearchSuggestItem as Qo, NxAppProperties as Qr, ChildListDef as Qt, GenericObjectLayoutNodeLevel2 as R, NxSelectionInfo as Ra, NxFieldResourceId as Ri, FieldInTableData as Rn, SearchFieldSelectionMode as Ro, MeasureListDef as Rr, ValueExpr as Rs, BookmarkApplyAndVerifyResult as Rt, ModalFunctions as S, NxMiniChartRows as Sa, NxDerivedField as Si, DriveType as Sn, QRange as So, HyperCube as Sr, TableRow as Ss, AlternateStateData as St, StandardFunctions as T, NxPage as Ta, NxDerivedGroup as Ti, EmbeddedSnapshotDef as Tn, ReloadError as To, InterFieldSortData as Tr, TableViewCtlSaveInfo as Ts, AppObjectListDef as Tt, GenericObjectPropertiesNode as U, NxStackedPivotCell as Ua, NxGetBookmarkOptions as Ui, FieldScores as Un, SearchGroupItemOptions as Uo, NativeFieldFunctions as Ur, BookmarkFieldVerifyWarning as Ut, GenericObjectLayoutNodeLevel5 as V, NxSortIndicatorType as Va, NxFieldTableResourceId as Vi, FieldListDef as Vn, SearchGroupItem as Vo, MetaData as Vr, VariableListDef as Vs, BookmarkFieldPageEx as Vt, GenericObjectPropertiesNodeLevel2 as W, NxStateCounts as Wa, NxGetObjectOptions as Wi, FieldType as Wn, SearchGroupItemType as Wo, NativeGenericBookmarkFunctions as Wr, BookmarkList as Wt, UndoInfoObject as X, NxTreeDimensionInfo as Xa, NxHighlightRanges as Xi, FolderItem as Xn, SearchPage as Xo, NativeVariableFunctions as Xr, CharRange as Xt, GenericVariable as Y, NxTreeDimensionDef as Ya, NxGrpType as Yi, FilterType as Yn, SearchObjectOptions as Yo, NativeGlobalFunctions as Yr, CharEncodingType as Yt, RpcObject as Z, NxTreeMultiRangeSelectInfo as Za, NxHypercubeMode as Zi, FolderItemType as Zn, SearchResult as Zo, NxAppLayout as Zr, ChildList as Zt, QixProgressMessage as _, NxMetaDef as _a, NxContinuousRangeSelectInfo as _i, DimensionReference as _n, ProgressData as _o, GenericVariableType as _r, StringExpression as _s, StoryListData as _t, AppSession as a, NxLibraryMeasure as aa, NxAutoSortByStateDef as ai, CyclicGroupPosition as an, NxVariableListItem as ao, GenericDimensionLayout as ar, SelectionObject as as, ListBoxProperties as at, Doc as b, NxMiniChartData as ba, NxDataPage as bi, DocListEntry as bn, QIX_SCHEMA_VERSION as bo, GroupState as br, TableProfilingData as bs, VariableListItem as bt, NativeReactHooks as c, NxListObjectExpression as ca, NxBookmark as ci, DataTable as cn, ObjectInterface as co, GenericMeasureProperties as cr, SortCriteria as cs, MasterObjectListItem as ct, WorkloadType as d, NxLocalizedWarningCode as da, NxCell as di, DatabaseInfo as dn, OtherLimitMode as do, GenericObjectLayoutAdditionalProperties as dr, StateFieldValues as ds, ODAGAppLinkListData as dt, NxInlineMeasureDef as ea, NxAttrDimInfo as ei, CondDef as en, NxTreeValue as eo, GenericBookmarkEntry as er, SearchTermResult as es, DimensionListData as et, ProgressListener as f, NxMatchingFieldInfo as fa, NxCellPosition as fi, DatabaseOwner as fn, OtherMode as fo, GenericObjectProperties as fr, StaticContentList as fs, ODAGAppLinkListItem as ft, QixProgressEvent as g, NxMeta as ga, NxContinuousMode as gi, DimensionListDef as gn, PositionMark as go, GenericVariableProperties as gr, StringExpr as gs, SheetListItem as gt, QixProgressErrorSeverity as h, NxMeasureInfo as ha, NxContinuousDataOptions as hi, DimensionList as hn, Point as ho, GenericVariableLayout as hr, StaticContentUrlDef as hs, SheetListDataCellBounds as ht, withHostConfig as i, NxLibraryDimensionDef as ia, NxAttributeExpressionValues as ii, CustomConnector as in, NxValidationError as io, GenericDimensionInfo as ir, SelectInfo as is, ExpandedFieldListItem as it, FieldListObject as j, NxPivotDimensionCell as ja, NxDownloadOptions as ji, ExtensionList as jn, SearchCharRange as jo, LayoutFieldInfo as jr, TransformAppParameters as js, ArrayOfNxValuePoint as jt, CurrentSelections as k, NxPatchOperationType as ka, NxDimensionType as ki, ExtendedLayoutBookmarkData as kn, SearchAssociationResult as ko, LayoutBookmarkData as kr, TextMacro as ks, ApplyGroupStateWarningType as kt, OpenAppSessionProps as l, NxListObjectExpressionDef as la, NxCalcCond as li, DataTableEx as ln, OdbcDsn as lo, GenericObjectEntry as lr, SourceKeyRecord as ls, MeasureListData as lt, QixProgressError as m, NxMeasure as ma, NxContainerEntry as mi, DerivedFieldsInTableData as mn, OtherTotalSpecProp as mo, GenericVariableConstraints as mr, StaticContentUrl as ms, SheetListDataCell as mt, openAppSession as n, NxLayoutErrors as na, NxAttrExprInfo as ni, ContentLibraryList as nn, NxTrendlineDef as no, GenericBookmarkProperties as nr, SearchValuePage as ns, DynamicAppViewListData as nt, CloseProps as o, NxLibraryMeasureDef as oa, NxAxisData as oi, DataField as on, NxVariableProperties as oo, GenericDimensionProperties as or, SelectionObjectDef as os, ListItem as ot, ProgressType as p, NxMatchingFieldMode as pa, NxCellRows as pi, DelimiterInfo as pn, OtherSortMode as po, GenericObjectPropertiesAdditionalProperties as pr, StaticContentListItem as ps, SheetListData as pt, GenericObjectPropertiesNodeLevel5 as q, NxTickCell as qa, NxGroupObjectId as qi, FileType as qn, SearchMatchCombination as qo, NativeGenericObjectFunctions as qr, BookmarkVariableItem as qt, qix as r, NxLibraryDimension as ra, NxAttributeDimValues as ri, ContentLibraryListItem as rn, NxTrendlineMode as ro, GenericConnectMachine as rr, SearchValueResult as rs, DynamicAppViewListItem as rt, DocAction as s, NxLinkedObjectInfo as sa, NxAxisTicks as si, DataRecord as sn, NxViewPort as so, GenericMeasureLayout as sr, Size as ss, MasterObjectListData as st, QixAPI as t, NxLTrendlineType as ta, NxAttrExprDef as ti, Connection as tn, NxTrendline as to, GenericBookmarkLayout as tr, SearchValueOptions as ts, DimensionListItem as tt, UseAppHook as u, NxLocalizedErrorCode as ua, NxCardinalities as ui, Database as un, OleDbProvider as uo, GenericObjectLayout as ur, StateEnumType as us, MeasureListItem as ut, CombinedWebSocketStateEvent as v, NxMiniChart as va, NxCurrentSelectionItem as vi, DoReloadExParams as vn, ProgressMessage as vo, GraphMode as vr, SymbolFrequency as vs, StoryListItem as vt, SpecialObjectFunctions as w, NxOrigin as wa, NxDerivedFieldsData as wi, EmbeddedSnapshot as wn, Rect as wo, InputFieldItem as wr, TableViewConnectionPointSaveInfo as ws, AppObjectList as wt, ListFunctions as x, NxMiniChartDef as xa, NxDataReductionMode as xi, DriveInfo as xn, QMediaList as xo, GroupStateInfo as xr, TableRecord as xs, AlfaNumString as xt, WebSocketEvent as y, NxMiniChartCell as ya, NxDataAreaPage as yi, DoReloadExResult as yn, QFunction as yo, GroupBookmarkData as yr, SymbolValue as ys, VariableListData as yt, GenericObjectLayoutNodeLevel3 as z, NxSimpleDimValue as za, NxFieldSelectionInfo as zi, FieldInTableProfilingData as zn, SearchFieldValueItem as zo, MediaListDef as zr, ValueExpression as zs, BookmarkFieldItem as zt };