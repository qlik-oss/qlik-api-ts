import { a as sortKeys, i as isNode, r as isBrowser, t as cleanFalsyValues } from "./utils-m8Ydgbd8.js";
...
const getPlatform = async (options = {}) => {
	...
	let productInfo;
	if (isBrowser()) try {
		const rootUrl = new URL(window.QlikMain?.getRootUrl() || "");
		const origin = hc.host ? new URL(hc.host)?.origin : window.location.origin;
		if (rootUrl && rootUrl.origin === origin) productInfo = window.QlikMain?.PRODUCT_INFO;
	} catch {}
	if (!productInfo) {
...
function serializeHostConfig$1(hostConfig) {
	const hostConfigToUse = removeDefaults(resolveHostConfig(hostConfig));
...
*/
const QLIK_CSRF_TOKEN_HEADER = "qlik-csrf-token";
/**
* Name of the API version header.
*/
const QLIK_API_VERSION_HEADER = "qlik-api-version";
...
async function getCsrfToken(hostConfig, noCache) {
	...
	const fetchCsrfToken = async () => {
		try {
			...
			})).headers.get(QLIK_CSRF_TOKEN_HEADER);
			if (!csrfToken) return "";
			return csrfToken;
		} catch (e) {
...
/**
* AngularJS interpolates {{ }} expressions found in text content when it compiles
* the DOM, so query-parameter values rendered on the page could be executed as
* expressions. Break up consecutive braces with a zero-width space so the text
* can never form an interpolation marker, even if it is copied out of an
* ng-non-bindable element.
*/
function escapeAngularJsExpressions(text) {
...
function handleOAuthCallback() {
	...
	if (urlParams.get("error")) {
		...
		element.innerText = escapeAngularJsExpressions(`<code>${JSON.stringify({
...
*/
function toCacheKey({ url, query, headers, serializedHostConfig, apiVersion }) {
	let cacheKey = url;
	...
	cacheKey = cacheKey.concat(`+api-version=${apiVersion}`);
	return cacheKey;
}
/**
...
/**
* The default `qlik-api-version` sent with every request.
* Pinned to the latest stable quarterly release at the time the specs were generated.
* Regenerated automatically by `pnpm generate-specs`.
*/
...
const FLAG_OVERRIDES_GZIP_PREFIX = "gzip:";
const FLAG_OVERRIDES_PLAIN_MAX_CHARS = 4096;
const FLAG_OVERRIDES_ENCODED_MAX_CHARS = 8100;
const FLAG_OVERRIDES_SAFE_KEY_PATTERN = /^[\x20-\x7E]+$/;
const sanitizeFlagOverrides = (overrides) => {
	const sanitized = {};
	const dropped = [];
	for (const [key, value] of Object.entries(overrides)) if (FLAG_OVERRIDES_SAFE_KEY_PATTERN.test(key) && typeof value === "boolean") sanitized[key] = value;
	else dropped.push(key);
	if (dropped.length > 0) console.warn(`qmfe-api: dropping ${dropped.length} feature flag override(s) with non-printable-ASCII keys or non-boolean values before forwarding to ui-config: ${JSON.stringify(dropped)}`);
	return sanitized;
};
const gzipAndEncode = async (value) => {
	const cs = new CompressionStream("gzip");
	const writer = cs.writable.getWriter();
	const compressedPromise = new Response(cs.readable).arrayBuffer();
	const writeDonePromise = writer.write(new TextEncoder().encode(value)).then(() => writer.close());
	const [compressedBuffer] = await Promise.all([compressedPromise, writeDonePromise]);
	const compressed = new Uint8Array(compressedBuffer);
	let binary = "";
	for (let i = 0; i < compressed.length; i++) binary += String.fromCharCode(compressed[i]);
	return FLAG_OVERRIDES_GZIP_PREFIX + btoa(binary);
};
/**
* Encodes feature flag overrides for the x-feature-flag-overrides header.
* Entries with non-printable-ASCII keys or non-boolean values are dropped with
* a console warning (fetch would reject them as header content). Returns plain
* JSON for small sets, a "gzip:"-prefixed base64 gzip payload for large ones,
* or null (with a console warning) when nothing sendable remains, the value
* cannot be kept within header size limits, or compression fails. Never throws:
* a failure to build the header must not fail the ui-config request itself.
* @internal exported for testing only, stripped from the published types
*/
async function encodeFlagOverridesHeaderValue(overrides) {
	const sanitized = sanitizeFlagOverrides(overrides);
	if (Object.keys(sanitized).length === 0) return null;
	const json = JSON.stringify(sanitized);
	if (json.length <= FLAG_OVERRIDES_PLAIN_MAX_CHARS) return json;
	if (typeof CompressionStream === "undefined") {
		if (json.length <= FLAG_OVERRIDES_ENCODED_MAX_CHARS) return json;
		console.warn(`qmfe-api: feature flag overrides are ${json.length} chars, exceeding the header budget, and CompressionStream is unavailable - not forwarding overrides to ui-config`);
		return null;
	}
	let encoded;
	try {
		encoded = await gzipAndEncode(json);
	} catch (error) {
		console.warn("qmfe-api: failed to compress feature flag overrides - not forwarding overrides to ui-config", error);
		return null;
	}
	if (encoded.length > FLAG_OVERRIDES_ENCODED_MAX_CHARS) {
		console.warn(`qmfe-api: feature flag overrides are ${encoded.length} chars even after compression, exceeding the header budget - not forwarding overrides to ui-config`);
		return null;
	}
	return encoded;
}
...
function addDefaultInterceptors() {
	...
	if (isBrowser()) {
		...
		const uiConfigOverridesInterceptor = async (request, proceed) => {
			if (request.pathTemplate === "/api/v1/ui-config") {
				const overrides = {
					...flagsFromLocalStorage,
					...flagsFromUrl
				};
				if (Object.keys(overrides).length > 0) {
					const headerValue = await encodeFlagOverridesHeaderValue(overrides);
					if (headerValue !== null) return proceed({
						...request,
						options: {
							...request.options,
							headers: {
								...request.options?.headers,
								"x-feature-flag-overrides": headerValue
							}
						}
					});
				}
		...
		GLOBAL_INTERCEPTORS.addInterceptor(uiConfigOverridesInterceptor);
		GLOBAL_INTERCEPTORS.addInterceptor(coreBootInterceptor);
		const isProdHost = () => {
			try {
				const host = window.location.hostname;
				return host === "qlikcloud.com" || host.endsWith(".qlikcloud.com");
			} catch {
				return true;
			}
		};
		const readSignadotRoutingKey = () => {
			if (isProdHost()) return null;
			try {
				const raw = localStorage.getItem("qcs-signadot-key");
		...
		const upsertSignadotBaggage = (existing, key) => {
			const entry = `sd-routing-key=${key}`;
			if (!existing) return entry;
			const kept = existing.split(",").map((part) => part.trim()).filter((part) => part.length > 0 && !part.startsWith("sd-routing-key="));
			kept.push(entry);
			return kept.join(",");
		};
		const signadotInterceptor = async (request, proceed) => {
			const key = readSignadotRoutingKey();
			if (!key) return proceed(request);
			const existingHeaders = request.options?.headers || {};
			return proceed({
				...request,
				options: {
					...request.options,
					headers: {
						...existingHeaders,
						baggage: upsertSignadotBaggage(existingHeaders.baggage, key)
					}
				}
			});
		};
		GLOBAL_INTERCEPTORS.addInterceptor(signadotInterceptor);
	}
...
async function performActualHttpFetch(method, completeUrl, unencodedBody, contentType, options, authHeaders, credentials, userAgent, apiVersion, isPublic) {
	...
	if (apiVersion) headers[QLIK_API_VERSION_HEADER] = apiVersion;
	const request = {
		method,
		credentials,
		mode: "cors",
	...
	handleDeprecationHeaders(completeUrl, invokeFetchResponse, !!isPublic);
	return invokeFetchResponse;
}
/**
* Emits a console warning/debug message when the server signals that the called endpoint is
* deprecated, but only when the developer is likely to act on it:
* - internal in the browser and devtools mode is active (`localStorage.devtools === "true"`)
* - non-browser usage is not gated by browser devtools mode
* - public usage logs through debug instead of warning
*/
function handleDeprecationHeaders(url, response, isPublic) {
	if (isBrowser()) {
		if (!isPublic && globalThis.localStorage?.getItem("devtools") !== "true") return;
	}
	const deprecation = response.headers.get("deprecation");
	if (!deprecation) return;
	const sunset = response.headers.get("sunset");
	const link = response.headers.get("link");
	const parts = [`[Qlik] API deprecation notice for ${url} (deprecated since: ${parseSfDate(deprecation)})`];
	if (sunset) parts.push(`Removal no earlier than: ${sunset}`);
	if (link) parts.push(`Details: ${link}`);
	const message = parts.join(". ");
	if (isPublic) console.debug(message);
	else console.warn(message);
}
/**
* Parses an RFC 8941 sf-date value (e.g. `"@1688169599"`) into a `YYYY-MM` string.
* Returns the raw value unchanged if it does not match the expected format.
*/
...
async function getInvokeFetchUrlParams({ method, pathTemplate, pathVariables, query, options, apiVersion }) {
...
function invokeFetchWithUrl(api, props) {
	return invokeFetchWithUrlAndRetry(api, props, async () => {
		const { headers: authHeaders, credentials } = await getRestCallAuthParams({
			hostConfig: props.options?.hostConfig,
			method: props.method
		});
...
function invokeFetchWithUrlAndRetry(api, props, performRetry) {
	const { method, completeUrl, body, options, authHeaders, credentials, cacheKey, contentType, userAgent, apiVersion, isPublic } = props;
	...
	const resultPromiseFromBackend = performActualHttpFetch(method, completeUrl, body, contentType, options, authHeaders, credentials, userAgent, apiVersion, isPublic);
	...
	const resultPromiseAfterPagingAddon = addPagingFunctions(api, {
		...props,
		value: resultAfterAuthenticationCheck
	});
	return updateCache(api, cachingContext, resultPromiseAfterPagingAddon);
}
/**
* Adds paging functions to the response object if there are paging links present in the response body
*/
function addPagingFunctions(api, { method, body, options, authHeaders, credentials, value, userAgent, apiVersion }) {
	return value.then((resp) => {
...
/** The set of valid quarter-month suffixes defined by the API versioning policy. */
const VALID_QUARTER_MONTHS = /* @__PURE__ */ new Set([
	"02",
	"05",
	"08",
	"11"
]);
/** Matches `YYYY-MM` where the year is 4 digits and the month is 2 digits. */
const API_VERSION_REGEX = /^(\d{4})-(0[1-9]|1[0-2])$/;
/**
* Returns `true` when `version` is a valid `qlik-api-version` value.
*
* Valid values are:
* - `"latest"` — always treated as acknowledging the most recent release; no deprecation headers are returned.
* - `"YYYY-MM"` where MM ∈ `{ "02", "05", "08", "11" }` — a stable quarterly release date.
*
* Invalid or unrecognised values are handled server-side by falling back to the oldest supported
* release, but passing them is still an error in the calling code.
*/
function isValidApiVersion(version) {
	if (version === "latest") return true;
	const match = API_VERSION_REGEX.exec(version);
	if (!match) return false;
	return VALID_QUARTER_MONTHS.has(match[2]);
}
/**
...
async function invokeFetchIntercepted(api, props) {
	...
	if (props?.apiVersion) {
		if (!isValidApiVersion(props.apiVersion)) {
			console.warn(`[qmfe-api] Invalid apiVersion "${props.apiVersion}". Must be "latest" or a quarterly release in YYYY-MM format (MM is {02, 05, 08, 11}). Using default version to "${API_VERSION}".`);
			apiVersion = API_VERSION;
		} else apiVersion = props.apiVersion;
	} else apiVersion = API_VERSION;
	const { completeUrl, authHeaders, credentials, cacheKey } = await getInvokeFetchUrlParams({
		...props,
		apiVersion
	});
...
//#endregion
export { registerHostConfig as A, exposeInternalApiOnWindow as B, isWindows as C, onPageRedirectRequested as D, onFatalAuthError as E, unregisterHostConfig as F, getPlatform as H, AuthorizationError as I, InvalidAuthTypeError as L, setDefaultHostConfig as M, toValidLocationUrl as N, onPageRedirectStarted as O, toValidWebsocketLocationUrl as P, InvalidHostConfigError as R, isHostCrossOrigin as S, normalizeHostConfig as T, generateRandomString as V, getDefaultHostConfig as _, EncodingError as a, getWebSocketAuthParams as b, addInterceptor as c, getInterceptors as d, interceptors as f, getAccessToken as g, determineAuthType as h, parseFetchResponse as i, serializeHostConfig as j, registerAuthModule as k, createInterceptors as l, appendQueryToUrl as m, invokeFetch as n, InvokeFetchError as o, removeInterceptor as p, isValidApiVersion as r, addDefaultInterceptors as s, clearApiCache as t, encodeFlagOverridesHeaderValue as u, getRestCallAuthParams as v, logout as w, handleAuthenticationError as x, getWebResourceAuthParams as y, UnexpectedAuthTypeError as z };