...
import { B as exposeInternalApiOnWindow, C as isWindows, P as toValidWebsocketLocationUrl, V as generateRandomString, b as getWebSocketAuthParams, m as appendQueryToUrl, x as handleAuthenticationError } from "./invoke-fetch-functions-C_Fj8sJ4.js";
import { n as getHumanReadableSocketClosedErrorMessage$1 } from "./qix-g-cGB4fY.js";
...
var engine_api_default = {
	...
	enums: {
		...
		LocalizedErrorCode: {
			...
			LOCERR_RELOAD_EXIT_ERROR: 11049,
...
const observableMethods = /* @__PURE__ */ new Set([
	"OpenDoc",
	"DoReload",
	"DoReloadEx",
	"DoSave"
...
/** @private */
const createRequestInterceptorForProgressMonitor = (monitor) => {
	return { onFulfilled(sessionReference, request) {
		const { method, id } = request;
		if (observableMethods.has(method)) monitor.startMonitoringProgress(method, id);
		return request;
	} };
};
/** @private */
const createResponseInterceptorForProgressMonitor = (monitor) => {
	return {
		onFulfilled(sessionReference, request, response) {
			const { method, id } = request;
			if (observableMethods.has(method)) monitor.stopMonitoringProgress(id);
			return response;
		},
		onRejected(sessionReference, request, error) {
			const { method, id } = request;
			if (observableMethods.has(method)) monitor.stopMonitoringProgress(id, "error");
...
JSONPatch$1.apply = function(original, patches) {
	patches.forEach((patch) => {
		...
		if (patch.op === "add" || patch.op === "replace") {
			...
			if (isArray$1(parent)) parent.splice(+key, 1);
	...
	extend: {
		getOrCreateSessionObject(props) {
			...
			if (!app._listCache[id]) {
				if (app.session.sessionState === "SESSION_ATTACHED") app._listCache[id] = app.getObject(id).catch(() => app.createSessionObject(props));
	...
	extend: { duplicate(story) {
		...
		return story.getLayout().then((layout) => {
			...
			for (let i = 0; i < items.length; i++) if (items[i].qInfo.qId === obj.id) {
				...
				if (next || prev) {
					if (next && !prev) rank = next.qData.rank - 1;
...
/** @internal */
async function createEnigmaSession({ appId, identity, hostConfig, useReloadEngine = false, ttlSeconds, workloadType }, progressMonitor) {
	...
	const session = enigma.create({
		...
		requestInterceptors: [somethingWithEmptyMethodsRequestInterceptor, createRequestInterceptorForProgressMonitor(progressMonitor)],
		responseInterceptors: [
			retryAbortedErrorResponseInterceptor,
			retryModalObjectError,
			somethingWithErrorPopupsResponnseInterceptor,
			createResponseInterceptorForProgressMonitor(progressMonitor)
		]
	});
...
JSONPatch.apply = function apply(original, patches) {
	patches.forEach((patch) => {
		...
		if (patch.op === "add" || patch.op === "replace") {
			...
			if (isArray(parent)) parent.splice(+key, 1);
			else delete parent[key];
		}
	});
};
		...
		onMessage: (event) => {
			...
			if (isInvocationResponse(data)) {
				...
				if (data.method === "OnConnected") resolveConnectionInfo(data.params?.qSessionState);
					...
					if (data.mustAuthenticate) handleAuthenticationError({
						headers: new Headers(),
						status: 101,
						canRetry: true,
						hostConfig
...
var PhoenixRpcObjectImpl = class extends EventEmitter {
	...
	*/
	...
	#createAlterEgoWithMixins(mixins) {
		const sourcePrototype = getPrototypeJustBeforeBase(this);
		return new (getGeneratedMixinClass(mixins, sourcePrototype, this.type, this.genericType))(this, mixins);
	}
};
...
function generateClassWithMixins(effectiveMixins, sourcePrototype, type, genericType) {
	const CustomPromise = [...effectiveMixins].toReversed().find((mixin) => mixin.Promise)?.Promise || Promise;
	const wrapInPromise = getPromiseWrapper(CustomPromise);
		...
		if (typeof value === "function") {
			Object.defineProperty(newPrototype, key, {
				value: function wrappedForwarder(...args) {
					const result = value.apply(this.__original_rpc_object, args);
					return wrapInPromise(wrapWithMixins(result));
				},
				configurable: true,
				writable: true
			});
...
*/
async function createPhoenixConnection(openProps, objectFactory, { onWebSocketEvent, getInitialAppActions, progressMonitor }) {
...
var PhoenixConnectionImpl = class {
	...
	*/
	...
	async #openNewQixConnectionAndPrepareSessionState(lastState) {
		...
		const newConnection = await createQixConnection(this.#openProps, {
			onMessageSent: (message, handle) => {
				this.#owner.progressMonitor.startMonitoringProgress(message.method, message.id);
			...
			onMessageReceived: (message, handle) => {
				...
				if (message?.id && typeof message.id === "number") this.#owner.progressMonitor.stopMonitoringProgress(message.id, message.error ? "error" : "done");
		...
		this.#owner.progressMonitor.setGetProgressFn((requestId) => newConnection.invoke(-1, {
			method: "GetProgress",
			params: [requestId],
			outKey: "qProgressData"
		}));
	...
	#onConnectionClosed(closeEvent) {
		...
		this.#owner.progressMonitor.stopMonitoringProgress(void 0, "aborted");
...
async function createEnigmaSessionEntrypoint(openProps, monitor) {
	return createEnigmaSession(openProps, monitor);
}
