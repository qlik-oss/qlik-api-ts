...
async function createEnigmaSession({
  ...
  if (isNodeEnvironment) {
    const { headers, queryParams } = await _chunkXASXL2NCjs.getRestCallAuthParams.call(void 0, { hostConfig, method: "POST" });
    ...
    const { queryParams } = await _chunkXASXL2NCjs.getWebSocketAuthParams.call(void 0, { hostConfig });
    Object.entries(queryParams).forEach(([key, value]) => {
      url = `${url}&${key}=${value}`;
    });
