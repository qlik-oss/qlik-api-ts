...
async function createAndSetupEnigmaSession(props, canRetry) {
  const { createEnigmaSession } = await Promise.resolve().then(() => _interopRequireWildcard(require("./enigma-session-SX3S4D67.js")));
  const session = await createEnigmaSession(props);
  setupSessionListeners(session, props);
  let global;
  if (await _chunk2EGD2BDXjs.isWindows.call(void 0, props.hostConfig)) {
    ...
    if (_optionalChain([loginInfo, 'optionalAccess', _ => _.mustAuthenticate])) {
      const action = await _chunk2EGD2BDXjs.handleAuthenticationError.call(void 0, {
        headers: new Headers(),
        status: 101,
        canRetry,
...
async function checkConnectivity(hostConfig) {
  ...
  const fetchRoot = _chunk2EGD2BDXjs.invokeFetch.call(void 0, "", { method, pathTemplate: "", options }).catch(catchFunc);
  const fetchMe = _chunk2EGD2BDXjs.invokeFetch.call(void 0, "", { method, pathTemplate: "/api/v1/users/me", options }).catch(catchFunc);
...
async function createSessionApp() {
  let sharedSession;
  if ((await _chunk2EGD2BDXjs.getPlatform.call(void 0, )).isCloud) {
