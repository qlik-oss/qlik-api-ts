...
function toGlobalAppSessionId({ appId, identity, hostConfig }) {
  const locationUrl = _chunkXASXL2NCjs.toValidEnigmaLocationUrl.call(void 0, hostConfig);
...
async function createAndSetupEnigmaSession(props, canRetry) {
  const { createEnigmaSession } = await Promise.resolve().then(() => _interopRequireWildcard(require("./enigma-session-TEB53FAF.js")));
  const session = await createEnigmaSession(props);
  setupSessionListeners(session, props);
  let global;
  if (await _chunkXASXL2NCjs.isWindows.call(void 0, props.hostConfig)) {
    ...
    if (_optionalChain([loginInfo, 'optionalAccess', _ => _.mustAuthenticate])) {
      const action = await _chunkXASXL2NCjs.handleAuthenticationError.call(void 0, {
        headers: new Headers(),
        status: 101,
        canRetry,
...
async function checkConnectivity(hostConfig) {
  ...
  const fetchRoot = _chunkXASXL2NCjs.invokeFetch.call(void 0, "", { method, pathTemplate: "", options }).catch(catchFunc);
  const fetchMe = _chunkXASXL2NCjs.invokeFetch.call(void 0, "", { method, pathTemplate: "/api/v1/users/me", options }).catch(catchFunc);
...
async function createSessionApp() {
  let sharedSession;
  if ((await _chunkXASXL2NCjs.getPlatform.call(void 0, )).isCloud) {
