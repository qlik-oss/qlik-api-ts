...
export type BatchPredictionScheduleInputAttributes = {
   ...
   * datasets. */
  applyDatasetChangeOnly: boolean;
  /** When the job is scheduled to finish, date needs to be now or in future */
  endDateTime?: string;
  /** Recurrence rules. Maximum is DAILY but you can specify the
   * hour, minute, and second it runs each day.
   * One string per rule. */
  recurrence: string[];
  /** When the job is scheduled to start, date needs to be now or in future */
  startDateTime: string;
  /** Timezone used for the date-time fields */
  timezone: string;
};
...
export type Model = {
  ...
  /** Name of this entity */
  name?: EntityName;
  powerTransformFeatures?: string[] | null;
