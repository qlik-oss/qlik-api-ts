...
async function createAndSetupEnigmaSession(props, canRetry) {
  const { createEnigmaSession } = await Promise.resolve().then(() => _interopRequireWildcard(require("./enigma-session-VPPJMBEM.js")));
  const session = await createEnigmaSession(props);
  setupSessionListeners(session, props);
  let global;
  if (await _chunkRMKP2BDXjs.isWindows.call(void 0, props.hostConfig)) {
    ...
    if (_optionalChain([loginInfo, 'optionalAccess', _ => _.mustAuthenticate])) {
      const action = await _chunkRMKP2BDXjs.handleAuthenticationError.call(void 0, {
        headers: new Headers(),
        status: 101,
        canRetry,
...
async function checkConnectivity(hostConfig) {
  ...
  const fetchRoot = _chunkRMKP2BDXjs.invokeFetch.call(void 0, "", { method, pathTemplate: "", options }).catch(catchFunc);
  const fetchMe = _chunkRMKP2BDXjs.invokeFetch.call(void 0, "", { method, pathTemplate: "/api/v1/users/me", options }).catch(catchFunc);
...
async function createSessionApp() {
  let sharedSession;
  if ((await _chunkRMKP2BDXjs.getPlatform.call(void 0, )).isCloud) {
